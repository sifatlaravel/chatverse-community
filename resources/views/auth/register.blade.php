...
<form method="POST" action="{{ route('register.store') }}" class="space-y-4">
  ...
  <div>
    ...
    <input name="name" value="{{ old('name') }}" class="mt-1 w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3" required>
    @error('name')<div class="mt-1 text-xs text-rose-300">{{ $message }}</div>@enderror
  </div>
  <div>
    <label class="text-sm text-slate-300">Company (optional)</label>
    <input name="company" value="{{ old('company') }}" class="mt-1 w-full rounded-2xl border border-white/10 bg-black/30 px-4 py-3">
    @error('company')<div class="mt-1 text-xs text-rose-300">{{ $message }}</div>@enderror
  </div>
  <div>
  ...
  <button class="w-full rounded-2xl bg-gradient-to-r from-cyan-400 to-fuchsia-500 px-4 py-3 font-semibold text-slate-950">Create account</button>
  <div class="text-sm text-slate-400">Already have an account? <a class="text-white underline" href="{{ route('login') }}">Login</a></div>
</form>
